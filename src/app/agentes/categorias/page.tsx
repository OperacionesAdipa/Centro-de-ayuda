...
export default function CategoriasPage() {
  ...
  async function moveArticleUp(art: Article, secArticles: Article[]) {
    ...
    if (idx === 0) return
    const prev = secArticles[idx - 1]
    await Promise.all([
      fetch(`/api/agent/articles/${art.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ position: prev.position ?? idx - 1 }) }),
      fetch(`/api/agent/articles/${prev.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ position: art.position ?? idx }) }),
    ])
    await loadData()
  }
  ...
  async function moveArticleDown(art: Article, secArticles: Article[]) {
    ...
    const next = secArticles[idx + 1]
    await Promise.all([
      fetch(`/api/agent/articles/${art.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ position: next.position ?? idx + 1 }) }),
      fetch(`/api/agent/articles/${next.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ position: art.position ?? idx }) }),
    ])
    await loadData()
  }
