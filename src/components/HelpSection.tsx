  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
      <div style={{ padding: '14px 18px', borderRadius: 12, border: '0.5px solid var(--border)', background: 'linear-gradient(135deg, #f5f3ff, #fff)', display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--dark)' }}>{T('needTutorial')}</div>
        <div style={{ fontSize: 12, color: 'var(--muted)', lineHeight: 1.5 }}>{T('tutorialDesc')}</div>
        {!showForm && !sent && (
        ...
        {sent && (
          <div style={{ fontSize: 12, color: '#3b6d11', background: '#eaf3de', padding: '6px 10px', borderRadius: 8 }}>
            {T('requestSent')}
            <button className="help-again-btn" onClick={resetForm} style={{ marginLeft: 8 }}>{T('requestAnother')}</button>
          </div>
        )}
        {showForm && !sent && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 2 }}>
            <input className="tutorial-input" type="text" placeholder={T('yourName')} value={name} onChange={(e) => setName(e.target.value)} />
            <input className="tutorial-input" type="email" placeholder={T('yourEmail')} value={email} onChange={(e) => setEmail(e.target.value)} />
            <textarea className="tutorial-input" placeholder={T('whatTutorial')} value={desc} onChange={(e) => setDesc(e.target.value)} rows={2} />
            <div style={{ display: 'flex', gap: 8 }}>
              <button className="help-card-btn purple" onClick={sendRequest} disabled={sending}>
                {sending ? T('sending') : T('send')}
              </button>
              <button className="help-card-btn outline" onClick={() => setShowForm(false)}>{T('cancel')}</button>
            </div>
          </div>
        )}
      </div>
      ...
      <div style={{ padding: '14px 18px', borderRadius: 12, border: '0.5px solid var(--border)', background: 'linear-gradient(135deg, #eff8ff, #fff)', display: 'flex', flexDirection: 'column', gap: 8 }}>
        <span style={{ fontSize: 22 }}>📅</span>
        <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--dark)' }}>{T('preferTalk')}</div>
        <div style={{ fontSize: 12, color: 'var(--muted)', lineHeight: 1.5 }}>{T('talkDesc')}</div>
