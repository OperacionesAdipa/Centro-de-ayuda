...
export function ActualizarVideos() {
  ...
  async function regenerateFromVideo(videoId: number) {
    ...
    if (res.ok) {
      setRegenerateResult(prev => ({ ...prev, [videoId]: `Se actualizaron ${data.updated.length} artículo(s) y quedaron pendientes de revisión` }))
      await loadData()
    } else {
          ...
          <div key={v.id} className="agent-side-card" style={{ gap: 0, padding: 0, overflow: 'hidden', borderTop: '3px solid #0ea5e9' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px', gap: 12, flexWrap: 'wrap' }}>
              <div style={{ display: 'flex', gap: 14, alignItems: 'center', flex: 1 }}>
                <div style={{ width: 80, height: 50, background: 'linear-gradient(135deg, #0ea5e9, #704EFD)', borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 20, flexShrink: 0 }}>&#9654;</div>
                <div>
                  <div style={{ fontWeight: 600, fontSize: 14, color: 'var(--dark)', marginBottom: 2 }}>{v.title}</div>
                  <a href={v.vimeo_url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, color: '#0ea5e9' }}>{v.vimeo_url}</a>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                ...
                <button
                  style={{ background: '#0ea5e9', color: '#fff', border: 'none', borderRadius: 99, padding: '5px 12px', fontSize: 12, cursor: 'pointer' }}
                  onClick={() => regenerateFromVideo(v.id)}
                  disabled={regenerating === v.id || v.articles.length === 0}
                >
                  {regenerating === v.id ? 'Actualizando...' : 'Actualizar artículos'}
                </button>
            ...
            {regenerateResult[v.id] && (
              <div style={{ padding: '8px 18px', fontSize: 13, background: regenerateResult[v.id].startsWith('Error') ? '#fcebeb' : '#e0f2fe', color: regenerateResult[v.id].startsWith('Error') ? '#a32d2d' : '#0284c7' }}>
                {regenerateResult[v.id]}
              </div>
            )}
